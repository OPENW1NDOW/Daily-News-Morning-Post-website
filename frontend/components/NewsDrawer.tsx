...
export function NewsDrawer({ item, open, onClose, onFavoriteToggle, categories }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent>
        {/* 固定头部（右上留出空间给 Dialog 自带的 × 按钮） */}
        <div className="shrink-0 px-6 pt-6 pb-4 border-b border-stone-100 pr-14">
          <div className="flex items-center gap-2 flex-wrap mb-3">
            <span className="text-xs font-medium text-[#2563EB] bg-[#2563EB]/10 px-2 py-0.5 rounded-md">
              {categoryName}
            </span>
            <span className="text-xs text-[#737373]">{item.date}</span>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6 text-sm">
          ...
          {item.full_summary && (
            <section>
              <h3 className="text-xs font-semibold text-[#737373] uppercase tracking-wider mb-2">
                详细总结
              </h3>
              ...
              <ul className="space-y-3">
                {item.viewpoints.map((vp, i) => (
                  <li key={i} className="border-l-2 border-[#2563EB]/30 pl-3">
                    ...
                    {vp.source && (
                      <p className="text-xs text-[#737373] mt-1">— {vp.source}</p>
                    )}
                  </li>
                ))}
              </ul>
