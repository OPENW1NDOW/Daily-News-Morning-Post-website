...
export function NewsCard({ item, categories, onClick, onFavoriteToggle }: Props) {
  ...
  return (
    <article
      ...
      className="group relative flex flex-col bg-white border border-stone-200 rounded-2xl p-6 cursor-pointer transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(15,15,15,0.06)] hover:border-stone-300 overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-400 focus-visible:ring-offset-2"
    >
      {/* 左侧细彩条 */}
      <span
      ...
      <div className="flex items-center gap-1.5 mb-3">
        ...
        {isImportant && (
          <span className="text-[10px] text-[#737373] font-medium">· 重要</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 pt-3 mt-auto border-t border-stone-100 text-[11.5px] text-[#737373]">
        <span className="truncate">{sourceName}</span>
        {item.source_links && item.source_links.length > 1 && (
          <span className="text-[11px] text-[#737373] shrink-0">· +{item.source_links.length - 1}</span>
        )}
      </div>
    </article>
  )
